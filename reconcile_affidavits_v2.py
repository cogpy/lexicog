#!/usr/bin/env python3
"""
Reconcile the CSV responses with the correct 141-paragraph AD reference from the database.
The CSV files have JR/DR numbers that correspond to AD paragraph numbers.
"""
import sqlite3
...
def get_existing_responses_from_csv(conn, response_type='jr'):
    """Get existing JR or DR responses from the CSV data."""
    cursor = conn.cursor()
    table = 'jr_responses' if response_type == 'jr' else 'dr_responses'
    
    # The jr_number/dr_number column contains the AD paragraph number
    cursor.execute(f'''
        SELECT jr_number, response_text
        FROM {table}
        ORDER BY id
    ''' if response_type == 'jr' else f'''
        SELECT dr_number, response_text
        FROM {table}
        ORDER BY id
    ''')
    ...
    for row in cursor.fetchall():
        number, text = row
        if number:
            # The number in the CSV is the AD paragraph number (e.g., "7.1")
...
def generate_affidavit_v13(conn, response_type='jr'):
    ...
    # Get existing responses (keyed by AD paragraph number)
    existing_responses = get_existing_responses_from_csv(conn, response_type)
    
    print(f"  Found {len(existing_responses)} existing {response_type.upper()} responses")
...
"""
    ...
    for ad_number, seq_order, section_number in all_ads:
        ...
        if ad_number in existing_responses:
            # Use existing response
            response_text = existing_responses[ad_number]
            ...
            if ad_content:
                ...
                content_preview = ad_content[:150] + "..." if len(ad_content) > 150 else ad_content
                affidavit += f"**{prefix} {ad_number}** [RESPONSE REQUIRED] The Applicant states: \"{content_preview}\" This paragraph requires a detailed response.\n\n"
            else:
