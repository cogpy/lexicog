...
def create_database_schema(conn):
    ...
    # Table 2: AD Paragraphs (from CSV - flexible schema)
    cursor.execute('''
...
def import_csv_generic(conn, table_name, csv_file):
    """Import a CSV file into the specified table with dynamic column mapping."""
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        if not column_mapping:
            print(f"⚠ No matching columns found for {csv_file}")
            return 0
...
def generate_reconciliation_report(conn):
    ...
    if extra:
        ...
        report += "These AD paragraphs exist in the CSV data but are not in the correct 141-paragraph reference:\n\n"
        for ad_num, in extra:
            report += f"- AD {ad_num}\n"
    ...
    report += f"- **JR Responses:** {jr_count}\n"
    report += f"- **DR Responses:** {dr_count}\n"
...
def main():
    ...
    try:
        ...
        # Import CSV files
        print("\nImporting CSV files...")
        import_csv_generic(conn, "ad_paragraphs_csv", CSV_FILES["ad_paragraphs"])
        import_csv_generic(conn, "affidavit_sections", CSV_FILES["affidavit_sections"])
        import_csv_generic(conn, "comments", CSV_FILES["comments"])
        import_csv_generic(conn, "jr_responses", CSV_FILES["jr_responses"])
        import_csv_generic(conn, "dr_responses", CSV_FILES["dr_responses"])
