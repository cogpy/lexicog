...
var affidavitSections = mysqlTable("affidavit_sections", {
  id: int("id").autoincrement().primaryKey(),
...
var adParagraphs = mysqlTable("ad_paragraphs", {
  id: int("id").autoincrement().primaryKey(),
  sectionId: int("section_id").notNull(),
  paragraphNumber: varchar("paragraph_number", { length: 20 }).notNull().unique(),
  content: text("content").notNull(),
  summary: text("summary"),
  severity: int("severity").default(5),
  // 1-10 scale: 1=neutral (green), 10=serious (red)
  annexures: text("annexures"),
  // JSON array of annexure references
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var jrResponses = mysqlTable("jr_responses", {
  id: int("id").autoincrement().primaryKey(),
  sectionId: int("section_id").notNull(),
  paragraphNumber: varchar("paragraph_number", { length: 20 }).notNull().unique(),
  content: text("content").notNull(),
  evidenceStrength: int("evidence_strength").default(5),
  // 1-10 scale: strength of evidence
  annexures: text("annexures"),
  // JSON array of annexure references
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var drResponses = mysqlTable("dr_responses", {
  id: int("id").autoincrement().primaryKey(),
  sectionId: int("section_id").notNull(),
  paragraphNumber: varchar("paragraph_number", { length: 20 }).notNull().unique(),
  content: text("content").notNull(),
...
async function getAllAffidavitData() {
  const db = await getDb();
  if (!db) {
  ...
  for (const section of sections) {
    const adParas = await db.select().from(adParagraphs).where(eq(adParagraphs.sectionId, section.id));
    const jrResps = await db.select().from(jrResponses).where(eq(jrResponses.sectionId, section.id));
    const drResps = await db.select().from(drResponses).where(eq(drResponses.sectionId, section.id));
...
async function getComments(filters) {
  ...
  const query = conditions.length > 0 ? db.select().from(comments).where(and(...conditions)) : db.select().from(comments);
  return await query;
}
...
var appRouter = router({
  ...
  comments: router({
    getAll: publicProcedure.input(z2.object({
      sectionNumber: z2.string().optional(),
      paragraphNumber: z2.string().optional(),
      paragraphType: z2.enum(["AD", "JR", "DR"]).optional()
    })).query(async ({ input }) => {
      const comments2 = await getComments(input);
      return comments2;
    }),
    add: publicProcedure.input(z2.object({
      sectionNumber: z2.string(),
      paragraphNumber: z2.string(),
      paragraphType: z2.enum(["AD", "JR", "DR"]),
