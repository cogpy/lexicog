...
def fix_ad_placeholders(content, prefix):
    """Replace {ad_num} placeholders with actual AD paragraph numbers"""
    
    # Pattern to match response paragraphs like "DR 7.1" or "JR 10.5.2"
    pattern = rf'\*\*{prefix} (\d+(?:\.\d+)*)\*\* (.*?AD {{ad_num}}.*?)(?=\n\n\*\*{prefix}|\n\n###|\Z)'
    
    def replace_placeholder(match):
        para_num = match.group(1)
        content_text = match.group(2)
        # Replace {ad_num} with the actual AD paragraph number
        enhanced = content_text.replace('{ad_num}', para_num)
        return f'**{prefix} {para_num}** {enhanced}'
    
    result = re.sub(pattern, replace_placeholder, content, flags=re.DOTALL)
    return result
...
def process_affidavit(input_file, output_file, prefix):
    ...
    # Fix placeholders
    content = fix_ad_placeholders(content, prefix)
    ...
    print(f"  Remaining placeholders: {remaining_count}")
