#!/usr/bin/env python3
"""
Refine Jacqueline's affidavit for complete coverage, correct ordering, neutral tone, and proper citations.
"""
import re
import json
...
def generate_missing_response(ad_para_num, founding_content):
    ...
    # This is a simplified search, a more robust one would be needed for perfect context
...
def main():
    ...
    for para_num in ad_order:
        ...
        refined_affidavit += f"**JR {para_num}**\n\n"
        
        if response_data['jax_response'] != '[MISSING]':
            original_response = response_data['jax_response']
            refined_response = refine_text(original_response)
            # Add placeholder for citation
            refined_response += " [Annexure JR-X]"
    ...
    output_file = Path('/home/ubuntu/canima/affidavits_refined/Jacqueline_Answering_Affidavit_v13_REFINED.md')
    with open(output_file, 'w') as f:
        f.write("# Answering Affidavit of Jacqueline Faucitt (v13 - Refined)\n\n")
        f.write("**Case No:** 2025-137857\n")
        f.write("**Court:** High Court of South Africa, Gauteng Division, Pretoria\n\n")
        f.write("---\n\n")
        f.write(refined_affidavit)
        
    print(f"Refined Jacqueline's affidavit saved to: {output_file}")
